...
biForm中的标准控件都继承自 Qt 中的 QObject，QObject 提供的接口的帮助文档都可以从 Qt 的官方文档中找到，我们这里就不一一详细解释。

biForm中标准控件的接口继承自QObject的部分与 Qt 中的 QObject 的接口并不完全一样，有些接口也不推荐在使用标准控件时使用，本文只重点讲可以用得到的接口。
