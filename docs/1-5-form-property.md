...
同一个表单，在运行时状态下，在不同的数据库里，以不同的身份注册的表单，会有不同的 publishUUID，可以通过这个值区别不同的表单的运行时实例。

程序中一般较少用到这个属性，但可以通过这个属性值查询系统表中与这个表单相关的记录。

这个是只读属性，不允许修改，并且在一个数据库的实例中，经注册后的表单这个值是不变的。
...
在某些情况下，可能开发者并不希望表单上有这个工具栏，可以使用这个属性将之隐藏。
