...
dialog=DialogSelectItem(gbk('员工'),this.form)

#设置表格各列显示的标题
dialog.setTitles([gbk('ID'),gbk('员工编号'),gbk('姓名'),gbk('街道'),gbk('类别'),gbk('职务')])
...
#设置是否允许选择多个项目,传入参数为 False ，表示一次只允许选择一个荐目
...
def buSelectPerson_clicked():
    ...
    #运行这个对话框，用 dialog.exec() 也可以
    dialog.execDialog()

    #获取这个对话框运行时用户选择的条目，如果得到 None，表示用户点了“取消”，否则返回的是第一栏“ID”的值
