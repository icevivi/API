...
- [界面布局](#界面布局)
  - [1、自由布局](#1、自由布局)
  - [2、使用布局管理器](#2、使用布局管理器)
  - [3、使用可调式布局管理器](#3、使用可调式布局管理器)
  - [4、使用 Qt 的布局管理器](#4、使用 Qt 的布局管理器)
...
表单的属性比较重要，与表单运行时的状态有密切关系，以下分别讲解：
...
| 图片集                  | 表单上需要使用的图片都可以加入到图片集中，发布时这些图片也会包含到PFF文件中                                                                |
...
缺省状态下，表单运行时会自带一个工具栏，其中有一些缺省的工具按钮，还有记录定位器，如下图所示：

![toolbar](3-2-07.png)

工具栏上大部分按钮都只对设置了“ 主表”属性的表单有用，但也有少量功能比如“截图”、“打印”、“关闭”等是所有表单都可以用得上的，但这些功能也可以通过其它方式调用，或者使用脚本来调用，所以即使不使用缺省的工具栏，也不会影响这些表单的使用。对于不需要使用“主表”属性，也即不需要使用缺省的表单数据处理引擎的话，可以将“显示工具条”设为“否”。

但如果表单添加了自定义的“动作”，工具栏还是会显示，因为需要将这些动作添加到工具栏上供用户调用。

即使是设置了“主表”属性的表单，也可以不显示工具栏，可以通过 Python 脚本调用保存、删除、查询等接口实现这些工具栏按钮的功能。
